...
function defaultTeleportRegex() {
  return '([a-zA-Z0-9_\\u4e00-\\u9fa5]{1,16}) ?(请求传送|请求传送到你|想要传送到你|传送请求|wants to teleport to you|invites you to teleport to them|requests to teleport to you)\\.?';
}
...
function settingDefs() {
  return [
    ...
    { key: 'chat.botOwners', group: 'chat', type: 'textarea', label: '管理员名单', hint: '每行一个游戏名，可用 /tell <bot> 命令远程控制',
      extract: (ini) => {
