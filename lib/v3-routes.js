...
async function handleV3Route(ctx) {
  ...
  if (route === 'GET /api/v3/me') {
    return ok({ user: operator, role });
  }

  if (route === 'GET /api/v3/overview') {
    const list = health.getList();
    ...
    try {
      ...
      for (const node of nodes) {
        const r = await mcsm.request('GET', '/api/service/remote_service_instances', {
          daemonId: node.uuid, page: 1, page_size: 100
        });
        const list = (r.status === 200 && r.data && Array.isArray(r.data.data)) ? r.data.data : [];
  ...
  if (route === 'GET /api/v3/mcc/mod') {
    ...
    const antiAfk = parseSection('AntiAFK', ['enabled', 'delay', 'command']);
    const autoRelog = parseSection('AutoRelog', ['enabled', 'delay', 'retries', 'ignorekickmessage']);
