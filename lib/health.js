...
class HealthTracker {
  ...
  /**
   * 清理已不存在于实时实例列表中的历史条目（例如实例已在 MCSM 中删除）。
   * 仅移除 liveKeys 之外的键；keepDaemonIds 中的节点（实例列表查询失败）
   * 保守保留全部条目，防止节点临时不可用时误删统计。
   * @param {Set<string>|Array<string>} liveKeys 当前实时实例键集合（daemonId::uuid）
   * @param {Array<string>} [keepDaemonIds] 查询失败需整体保留的节点 id
   * @returns {Array<string>} 被移除的键
   */
  pruneStale(liveKeys, keepDaemonIds) {
    const live = liveKeys instanceof Set ? liveKeys : new Set(Array.isArray(liveKeys) ? liveKeys : []);
    const keep = new Set(Array.isArray(keepDaemonIds) ? keepDaemonIds : []);
    const removed = [];
    for (const key of Object.keys(this.data.instances || {})) {
      const inst = this.data.instances[key];
      if (keep.has(inst && inst.daemonId)) continue; // 节点查询失败：保守保留
      if (!live.has(key)) {
